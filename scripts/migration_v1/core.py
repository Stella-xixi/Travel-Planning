"""Core implementation for the repeatable Travel-Planning V1 data migration.
...
SCHEMA_VERSION = 1
ARCHIVE_MANIFEST_SCHEMA = "travel-planning.v1.archive-manifest@1"
INVENTORY_SCHEMA = "travel-planning.v1.migration-inventory@1"
VERIFY_SCHEMA = "travel-planning.v1.reconciliation-report@1"
STATE_SCHEMA = "travel-planning.v1.backfill-state@1"
MAPPING_SCHEMA = "travel-planning.v1.owner-mapping@1"
...
def _deterministic_id(prefix: str, source_key: str) -> str:
    digest = hashlib.sha256(f"travel-planning-v1:{prefix}:{source_key}".encode()).hexdigest()[:48]
...
def backfill(
    *,
    ...
    result: dict[str, Any] = {
        "schema": "travel-planning.v1.backfill-report@1",
        "generated_at": iso_now(),
        "dry_run": dry_run,
        "request_hash": request_hash,
        "high_water_cursor": inventory.get("high_water_cursor"),
