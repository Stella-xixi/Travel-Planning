...
def apply_grants(database_url: str, *, sql_path: Path = GRANTS_SQL) -> None:
    ...
    if not database_url.strip():
        raise RuntimeError("TRAVEL_PLANNING_POSTGRES_DSN is required")
    sql = sql_path.read_text(encoding="utf-8")
    if "travel_planning_runtime_grants" not in sql or "FORCE ROW LEVEL SECURITY" not in sql:
...
def main() -> int:
    ...
    apply_grants(os.environ.get("TRAVEL_PLANNING_POSTGRES_DSN", ""))
    return 0
