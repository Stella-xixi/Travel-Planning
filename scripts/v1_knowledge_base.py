"""Validate, deploy, and verify versioned Travel-Planning V1 knowledge bundles."""
...
DEFAULT_BUNDLE = (
    REPOSITORY_ROOT
    / "agent/travel_planning_agent_runtime/rag/curated/travel-planning-travel-basics-zh/manifest.json"
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for name in ("apply", "verify"):
        ...
        command.add_argument(
            "--api-url",
            default=os.environ.get("TRAVEL_PLANNING_KNOWLEDGE_API_URL", DEFAULT_API_URL),
            help="trusted Travel-Planning API base ending in /api/v1",
        )
        if name == "apply":
            command.add_argument(
                "--allow-public",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command in {"validate", "plan"}:
            ...
            return 0
        token = os.environ.get("TRAVEL_PLANNING_ACCESS_TOKEN", "").strip()
        if not token:
            raise KnowledgeBaseToolError(
                "TRAVEL_PLANNING_ACCESS_TOKEN is required and must come from a restricted operator session"
            )
