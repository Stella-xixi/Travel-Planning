"""Run the unified Travel-Planning V1 contract-to-delivery quality gate."""
...
def build_gate_commands(*, include_web_build: bool = True) -> tuple[GateCommand, ...]:
    ...
    commands = [
        ...
        GateCommand(
            "backend",
            (python, "-m", "ruff", "check", "backend/travel_planning_api/v1"),
        ),
        GateCommand(
            "backend",
            (
                ...
                "-m",
                "mypy",
                "backend/travel_planning_api/v1",
                "agent/travel_planning_agent_runtime/a2a",
                "agent/travel_planning_agent_runtime/providers",
                "agent/travel_planning_agent_runtime/rag",
                "agent/travel_planning_agent_runtime/runtime_v2",
                "--config-file",
                "pyproject.toml",
            ),
        ),
...
def validate_offline_migration_sql(sql: str) -> list[str]:
    ...
    required = {
        ...
        "share capability resolver": "create function travel_planning_resolve_share_tenant",
    }
...
def run_migration_gate() -> bool:
    ...
    environment["TRAVEL_PLANNING_POSTGRES_DSN"] = (
        "postgresql://travel_planning_migrator:offline-only@127.0.0.1/travel-planning"
    )
...
def run_commands(commands: Sequence[GateCommand], selected: set[str]) -> list[str]:
    ...
    for command in commands:
        ...
        if result.returncode:
            ...
            report_github_error(
                title=f"Travel-Planning {command.area} gate failed",
    ...
    if "migration" in selected and not run_migration_gate():
        ...
        report_github_error(
            title="Travel-Planning migration rendering failed",
...
def main() -> int:
    ...
    if failures:
        print(f"Travel-Planning V1 quality gate failed: {', '.join(failures)}", file=sys.stderr)
        report_github_error(
            title="Travel-Planning V1 quality gate failed",
            message=f"Failed areas: {', '.join(failures)}",
        )
        return 1
    print("Travel-Planning V1 quality gate passed.")
    return 0
