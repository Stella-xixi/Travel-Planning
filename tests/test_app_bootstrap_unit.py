...
from travel_planning_api.bootstrap_app import allowed_origins, create_web_application
from travel_planning_api.v1.operations import DependencyReadiness


def test_local_cors_defaults_and_explicit_override(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ENVIRONMENT", "dev")
    monkeypatch.delenv("TRAVEL_PLANNING_DEPLOYMENT_ENV", raising=False)
...
def test_secure_deployment_requires_explicit_cors(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TRAVEL_PLANNING_DEPLOYMENT_ENV", "production")
    monkeypatch.delenv("CORS_ORIGINS", raising=False)
...
def test_readiness_probes_dependencies_and_metrics_require_a_token(
    ...
    monkeypatch.setenv("TRAVEL_PLANNING_METRICS_TOKEN", metrics_token)
    app = create_web_application()
    ...
    assert 'travel_planning_dependency_ready{dependency="postgresql"} 1' in metrics.text
