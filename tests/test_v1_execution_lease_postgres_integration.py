...
from backend.travel_planning_api.v1.store import RunExecutionLeaseLost


@pytest.mark.asyncio
async def test_postgres_execution_lease_claim_recovery_and_fencing() -> None:
    dsn = os.getenv("TRAVEL_PLANNING_TEST_POSTGRES_DSN", "").strip()
    if not dsn:
        pytest.skip("set TRAVEL_PLANNING_TEST_POSTGRES_DSN to run PostgreSQL lease parity")
