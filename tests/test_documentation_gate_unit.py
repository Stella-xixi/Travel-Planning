...
def _minimum_tree(root: Path) -> None:
    ...
    (compose / "v1.env.example").write_text("TRAVEL_PLANNING_TEST_VALUE=enabled\n", encoding="utf-8")
    (compose / "v1.yaml").write_text("value: ${TRAVEL_PLANNING_TEST_VALUE:-enabled}\n", encoding="utf-8")
...
def test_documentation_gate_reports_broken_links_and_unused_env(tmp_path: Path) -> None:
    _minimum_tree(tmp_path)
    (tmp_path / "README.md").write_text("# Travel-Planning\n\n[Missing](docs/missing.md)\n", encoding="utf-8")
    (tmp_path / "deploy/compose/v1.env.example").write_text(
        "TRAVEL_PLANNING_UNUSED_VALUE=1\n",
        encoding="utf-8",
    )

    assert any("broken local link" in error for error in check_local_links(tmp_path))
    assert any("TRAVEL_PLANNING_UNUSED_VALUE" in error for error in check_compose_env_parity(tmp_path))
