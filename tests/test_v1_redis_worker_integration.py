...
from backend.travel_planning_api.v1.runtime import ExecutionResult, RunCoordinator, V1Runtime
from backend.travel_planning_api.v1.store import InMemoryPlatformStore
from backend.travel_planning_api.v1.worker import RedisRunWorker
...
async def test_real_redis_xautoclaim_recovers_and_acks_stale_dispatch() -> None:
    redis_url = os.getenv("TRAVEL_PLANNING_TEST_REDIS_URL", "").strip()
    if not redis_url:
        pytest.skip("set TRAVEL_PLANNING_TEST_REDIS_URL to run Redis reclaim integration")

    redis = Redis.from_url(redis_url, decode_responses=True)
    suffix = uuid4().hex
    stream = f"travel-planning:test:run-reclaim:{suffix}"
    group = f"travel-planning-test-{suffix}"
