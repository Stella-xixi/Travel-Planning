...
from backend.travel_planning_api.v1.postgres_store import PostgresPlatformStore
from backend.travel_planning_api.v1.share_models import ShareCreateRequest, ShareStatus
from backend.travel_planning_api.v1.share_service import ShareCapabilityInvalid, ShareService
from backend.travel_planning_api.v1.store import canonical_request_hash
...
async def test_postgres_share_projection_rotation_and_revocation_are_fenced() -> None:
    dsn = os.getenv("TRAVEL_PLANNING_SHARE_TEST_DSN", "").strip()
    if not dsn:
        pytest.skip("set TRAVEL_PLANNING_SHARE_TEST_DSN to run PostgreSQL share parity")
