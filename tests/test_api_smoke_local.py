...
def _install_fake_model_config_manager(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("travel_planning_api.bootstrap_app.get_model_config_manager", lambda: manager)
    monkeypatch.setattr("travel_planning_api.routes.model.get_model_config_manager", lambda: manager)
    monkeypatch.setattr("travel_planning_api.routes.session.get_model_config_manager", lambda: manager)
...
async def test_metrics_alias_and_rate_limit_config_smoke(monkeypatch):
    _install_fake_model_config_manager(monkeypatch)
    monkeypatch.setenv("TRAVEL_PLANNING_METRICS_PATH", "/internal/metrics")
    monkeypatch.setenv("TRAVEL_PLANNING_RATE_LIMIT_MAX_REQUESTS", "2")
    monkeypatch.setenv("TRAVEL_PLANNING_RATE_LIMIT_WINDOW_SECONDS", "60")
    ...
    try:
        ...
        monkeypatch.delenv("TRAVEL_PLANNING_METRICS_PATH", raising=False)
        monkeypatch.delenv("TRAVEL_PLANNING_RATE_LIMIT_MAX_REQUESTS", raising=False)
        monkeypatch.delenv("TRAVEL_PLANNING_RATE_LIMIT_WINDOW_SECONDS", raising=False)
        server_config.reload()
