...
from backend.travel_planning_api.v1.auth import require_principal
from backend.travel_planning_api.v1.models import Principal
from backend.travel_planning_api.v1.provider_routes import router
...
async def test_provider_metadata_endpoints_are_authenticated_and_secret_free(
    ...
    monkeypatch.setenv("TRAVEL_PLANNING_AMAP_WEB_KEY", secret)
    monkeypatch.setenv("TRAVEL_PLANNING_PROVIDER_ALLOWLIST", "amap")
    gateway = build_default_provider_gateway()
    app = FastAPI()
    app.state.travel_planning_provider_gateway = gateway
    ...
    assert "TRAVEL_PLANNING_AMAP_WEB_KEY" not in serialized
