...
from travel_planning_contracts.generate import build_schema_documents

from .samples import build_valid_contracts, valid_run_events

jsonschema = pytest.importorskip(
    "jsonschema", reason="install travel-planning-contracts[test] to validate JSON Schema documents"
)
...
def _schema_by_contract() -> dict[str, dict[str, Any]]:
    return {
        document["x-travel-planning-contract"]: document
...
def test_every_schema_is_valid_draft_2020_12_and_all_objects_are_closed(
    ...
    assert schema["x-travel-planning-contract"].endswith("@1")
