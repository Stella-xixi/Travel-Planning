...
from travel_planning_agent_runtime.rag.models import LicenseMetadata
...
async def test_postgres_ingest_retrieve_and_cross_tenant_filter():
    dsn = os.getenv("TRAVEL_PLANNING_RAG_TEST_DSN", "").strip()
    if not dsn:
        pytest.skip("TRAVEL_PLANNING_RAG_TEST_DSN is not configured")
