...
def test_embedding_environment_builder_is_explicit_and_https_in_production(
    ...
    for name in (
        "TRAVEL_PLANNING_EMBEDDING_ENDPOINT",
        "TRAVEL_PLANNING_EMBEDDING_MODEL",
        "TRAVEL_PLANNING_EMBEDDING_API_KEY",
    ):
        monkeypatch.delenv(name, raising=False)
    assert build_embedding_provider_from_env() is None

    monkeypatch.setenv("TRAVEL_PLANNING_EMBEDDING_ENDPOINT", "https://embed.example/v1/embeddings")
    with pytest.raises(RuntimeError, match="configured together"):
        build_embedding_provider_from_env()

    monkeypatch.setenv("TRAVEL_PLANNING_EMBEDDING_MODEL", "semantic-v1")
    monkeypatch.setenv("ENVIRONMENT", "production")
    monkeypatch.setenv("TRAVEL_PLANNING_EMBEDDING_ENDPOINT", "http://embed.internal/v1/embeddings")
