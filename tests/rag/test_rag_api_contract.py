...
from travel_planning_agent_runtime.rag import InMemoryKnowledgeRepository, KnowledgeService
from backend.travel_planning_api.v1.models import Principal
from backend.travel_planning_api.v1.rag_routes import router


def build_app(*, authenticated: bool = True) -> FastAPI:
    app = FastAPI()
    app.include_router(router, prefix="/api/v1")
    app.state.travel_planning_knowledge_service = KnowledgeService(InMemoryKnowledgeRepository())
    if authenticated:
