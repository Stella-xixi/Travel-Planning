...
def test_export_release_manifest_writes_versions_and_images(tmp_path):
    ...
    assert payload["applications"]["backend"]["image"].endswith("/Travel-Planning-backend")
    assert payload["applications"]["frontend"]["image"].endswith("/Travel-Planning-frontend")
    assert payload["applications"]["backend"]["image_tag"] == "v3.3.0"
    assert payload["applications"]["frontend"]["image_tag"] == "v3.3.0"
    assert payload["applications"]["backend"]["image_ref"].endswith("/Travel-Planning-backend:v3.3.0")
    assert payload["applications"]["frontend"]["image_ref"].endswith("/Travel-Planning-frontend:v3.3.0")
