"""Configuration module for Travel-Planning."""
...
class ServerConfig:
    ...
    def _apply_env_overrides(self) -> None:
        ...
        web["host"] = os.getenv("TRAVEL_PLANNING_WEB_HOST", web.get("host", "0.0.0.0"))
        web["port"] = _parse_int(os.getenv("TRAVEL_PLANNING_WEB_PORT", web.get("port", 38000)), 38000, 1)
        web["debug"] = _parse_bool(os.getenv("TRAVEL_PLANNING_WEB_DEBUG", web.get("debug", False)), False)

        env_cors = os.getenv("CORS_ORIGINS") or os.getenv("TRAVEL_PLANNING_CORS_ORIGINS")
        if env_cors:
            web["cors_origins"] = [item.strip() for item in env_cors.split(",") if item.strip()]

        frontend["port"] = _parse_int(
            os.getenv("TRAVEL_PLANNING_FRONTEND_PORT", frontend.get("port", 33001)),
            33001,
            1,
        )

        middleware["request_timeout_seconds"] = float(
            os.getenv("TRAVEL_PLANNING_REQUEST_TIMEOUT_SECONDS", middleware.get("request_timeout_seconds", 30.0))
        )
        middleware["rate_limit_max_requests"] = _parse_int(
            os.getenv("TRAVEL_PLANNING_RATE_LIMIT_MAX_REQUESTS", middleware.get("rate_limit_max_requests", 100)),
            100,
            1,
        )
        middleware["rate_limit_window_seconds"] = _parse_int(
            os.getenv("TRAVEL_PLANNING_RATE_LIMIT_WINDOW_SECONDS", middleware.get("rate_limit_window_seconds", 60)),
            60,
            1,
        )

        observability["metrics_enabled"] = _parse_bool(
            os.getenv("TRAVEL_PLANNING_METRICS_ENABLED", observability.get("metrics_enabled", True)),
            True,
        )
        observability["metrics_path"] = os.getenv(
            "TRAVEL_PLANNING_METRICS_PATH",
            observability.get("metrics_path", "/api/metrics"),
        )
        observability["structured_logging"] = _parse_bool(
            os.getenv("TRAVEL_PLANNING_STRUCTURED_LOGGING", observability.get("structured_logging", True)),
            True,
        )

        database["backend"] = _normalize_db_backend(
            os.getenv("TRAVEL_PLANNING_DB_BACKEND", database.get("backend", "file")),
            "file",
        )
        database["postgres_dsn"] = str(
            os.getenv("TRAVEL_PLANNING_POSTGRES_DSN", database.get("postgres_dsn", ""))
        ).strip()
        database["pool_min"] = _parse_int(
            os.getenv("TRAVEL_PLANNING_DB_POOL_MIN", database.get("pool_min", 1)),
            1,
            1,
        )
        database["pool_max"] = _parse_int(
            os.getenv("TRAVEL_PLANNING_DB_POOL_MAX", database.get("pool_max", 5)),
            5,
            1,
        )

        startup["fail_fast_validation"] = _parse_bool(
            os.getenv("TRAVEL_PLANNING_FAIL_FAST_STARTUP_VALIDATION", startup.get("fail_fast_validation", False)),
            False,
        )
