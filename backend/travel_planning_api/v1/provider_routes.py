...
async def provider_lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    yield
    gateway = getattr(app.state, "travel_planning_provider_gateway", None)
    product_runtime = getattr(app.state, "travel_planning_v1_runtime", None)
    runtime_gateway = (
...
def get_provider_gateway(request: Request) -> ProviderGateway:
    ...
    gateway = getattr(request.app.state, "travel_planning_provider_gateway", None)
    if gateway is None:
    ...
    if gateway is None:
        ...
        request.app.state.travel_planning_provider_gateway = gateway
    return gateway
