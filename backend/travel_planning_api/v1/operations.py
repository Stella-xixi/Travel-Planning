...
class OperationalMetrics:
    ...
    def render(self) -> str:
        ...
        lines = [
            "# HELP travel_planning_build_info Travel-Planning API build information.",
            "# TYPE travel_planning_build_info gauge",
            f'travel_planning_build_info{{version="{self._label(self.version)}"}} 1',
            "# HELP travel_planning_dependency_ready Whether a required dependency is ready.",
            "# TYPE travel_planning_dependency_ready gauge",
        ]
        for dependency, value in sorted(self._dependencies.items()):
            lines.append(f'travel_planning_dependency_ready{{dependency="{dependency}"}} {value}')
        lines.extend(
            [
                "# HELP travel_planning_http_requests_total HTTP requests by template and status class.",
                "# TYPE travel_planning_http_requests_total counter",
            ]
        )
        for (method, route, status_class), value in sorted(self._request_counts.items()):
            ...
            lines.append(f"travel_planning_http_requests_total{{{labels}}} {value}")
        lines.extend(
            [
                "# HELP travel_planning_http_request_duration_seconds HTTP request duration.",
                "# TYPE travel_planning_http_request_duration_seconds histogram",
            ]
        )
        for method, route in sorted(self._duration_counts):
            ...
            for bucket in self.BUCKETS:
                ...
                lines.append(
                    "travel_planning_http_request_duration_seconds_bucket"
            ...
            lines.append(f"travel_planning_http_request_duration_seconds_sum{{{labels}}} {safe_sum:.9g}")
            lines.append(f"travel_planning_http_request_duration_seconds_count{{{labels}}} {count}")
        return "\n".join(lines) + "\n"
