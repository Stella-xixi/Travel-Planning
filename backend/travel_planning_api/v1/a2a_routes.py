"""Authenticated FastAPI adapter for the Travel-Planning A2A 1.0 agent mesh."""
...
from travel_planning_agent_runtime.a2a.constants import MAX_A2A_HTTP_BODY_BYTES
from travel_planning_agent_runtime.a2a.handler import TravelPlanningA2ARequestHandler
from travel_planning_agent_runtime.a2a.models import A2AActor
from travel_planning_agent_runtime.a2a.registry import AgentExecutor, AgentRegistry, build_default_registry
from travel_planning_agent_runtime.a2a.service import TaskService
from travel_planning_agent_runtime.a2a.store import AgentTaskPersistence, InMemoryAgentTaskStore
...
class PrincipalContextBuilder(ServerCallContextBuilder):
    ...
    def build(self, request: Request) -> ServerCallContext:
        principal = getattr(request.state, "travel_planning_a2a_principal", None)
    ...
    *,
    ...
    selected_database_url = str(
        ...
        if database_url is not None
            ...
            os.getenv("TRAVEL_PLANNING_A2A_DATABASE_URL", "").strip()
            or os.getenv("TRAVEL_PLANNING_V1_DATABASE_URL", "").strip()
        )
    ).strip()
    if selected_store is None and selected_database_url:
        from travel_planning_agent_runtime.a2a.postgres_store import PostgresAgentTaskStore
        ...
        raise RuntimeError(
            "TRAVEL_PLANNING_A2A_DATABASE_URL or TRAVEL_PLANNING_V1_DATABASE_URL is required "
    ...
    dispatchers = {
        profile.interface_id: JsonRpcDispatcher(
            request_handler=TravelPlanningA2ARequestHandler(
...
async def get_a2a_runtime(request: Request) -> A2ARuntime:
    ...
    runtime = getattr(request.app.state, "travel_planning_v1_a2a_runtime", None)
    if runtime is None:
        product_runtime = getattr(request.app.state, "travel_planning_v1_runtime", None)
    ...
    if runtime is None:
        from travel_planning_agent_runtime.runtime_v2 import build_core_a2a_executors

        knowledge = getattr(request.app.state, "travel_planning_knowledge_service", None)
        ...
        request.app.state.travel_planning_v1_a2a_runtime = runtime
    if not isinstance(runtime, A2ARuntime):
        raise RuntimeError("travel_planning_v1_a2a_runtime must be an A2ARuntime")
    return runtime
...
async def a2a_jsonrpc(
    ...
    request.state.travel_planning_a2a_principal = principal
