"""HTTP and resumable SSE adapters for the Travel-Planning V1 API."""
...
def get_runtime(request: Request) -> V1Runtime:
    ...
    runtime = getattr(request.app.state, "travel_planning_v1_runtime", None)
    if runtime is None:
        runtime = build_default_v1_runtime()
        request.app.state.travel_planning_v1_runtime = runtime
        if runtime.a2a_runtime is not None:
            request.app.state.travel_planning_v1_a2a_runtime = runtime.a2a_runtime
        if runtime.knowledge_service is not None:
            request.app.state.travel_planning_knowledge_service = runtime.knowledge_service
    return runtime
