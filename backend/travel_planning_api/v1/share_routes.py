...
def get_share_service(request: Request) -> ShareService:
    ...
    service = getattr(request.app.state, "travel_planning_share_service", None)
    if service is not None:
        if not isinstance(service, ShareService):
            raise RuntimeError("travel_planning_share_service must be a ShareService")
        return service
    runtime = get_runtime(request)
    if runtime.share_service is None:
        runtime.share_service = ShareService(runtime.store)
    request.app.state.travel_planning_share_service = runtime.share_service
...
async def get_public_share_snapshot(
    ...
    session_token: Annotated[
        str,
        Header(alias="X-Travel-Planning-Share-Session", min_length=32, max_length=128),
    ],
) -> PublicShareSnapshotResponse:
    response.headers["Cache-Control"] = "no-store"
    response.headers["Referrer-Policy"] = "no-referrer"
