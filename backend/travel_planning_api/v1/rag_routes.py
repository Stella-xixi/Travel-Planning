...
async def get_knowledge_service(request: Request) -> KnowledgeService:
    ...
    service = getattr(request.app.state, "travel_planning_knowledge_service", None)
    if service is None:
    ...
    if service is not None:
        if not isinstance(service, KnowledgeService):
            raise RuntimeError("travel_planning_knowledge_service must be a KnowledgeService")
        return service

    database_url = (
        os.getenv("TRAVEL_PLANNING_RAG_DATABASE_URL", "").strip()
    ...
    embedding_provider = getattr(request.app.state, "travel_planning_embedding_provider", None)
    repository = PostgresKnowledgeRepository.from_database_url(
        database_url,
        embedding_provider=embedding_provider,
    )
    service = KnowledgeService(repository)
    request.app.state.travel_planning_knowledge_service = service
    return service
