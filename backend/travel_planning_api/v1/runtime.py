...
class OrchestratedWholeRunExecutor:
    ...
    async def execute(
        ...
        from travel_planning_agent_runtime.runtime_v2 import StructuredTripRequest
        from travel_planning_contracts.artifacts import TripBrief
        from travel_planning_contracts.validation import validate_contract
    ...
    async def _execute_answer(
        ...
        from travel_planning_agent_runtime.a2a.models import A2AActor, ArtifactInput
        from travel_planning_contracts.artifacts import TravelAnswer, TravelQuestion
        from travel_planning_contracts.common import ActorRef, SourceKind, SourceRef
        from travel_planning_contracts.validation import validate_contract
    ...
    async def submit(
        ...
        *,
        ...
        if command.type == "trip.replan":
            from travel_planning_contracts.artifacts import TripSnapshot
            from travel_planning_contracts.validation import validate_contract
        ...
        if not result.replayed and self.in_process_execution:
            task = asyncio.create_task(
                ...
                name=f"travel-planning-run-{result.run.run_id}",
            )
    ...
    async def resume(
        ...
        *,
        ...
        if not result.replayed and self.in_process_execution:
            task = asyncio.create_task(
                self._execute(result.run.run_id, principal),
                name=f"travel-planning-run-{result.run.run_id}-resume",
            )
            async with self._task_guard:
                self._tasks[result.run.run_id] = task
            task.add_done_callback(lambda _: asyncio.create_task(self._forget(result.run.run_id)))
    ...
    async def execute_existing(
        ...
        *,
        ...
        execution_task = asyncio.create_task(
            ...
            name=f"travel-planning-external-run-{run_id}-{lease.attempt}",
        )
        heartbeat_task = asyncio.create_task(
            ...
            name=f"travel-planning-run-heartbeat-{run_id}-{lease.attempt}",
        )
        try:
            try:
...
def build_default_v1_runtime() -> V1Runtime:
    ...
    database_url = os.getenv("TRAVEL_PLANNING_V1_DATABASE_URL", "").strip()
    ...
    if database_url:
        ...
        raise RuntimeError("TRAVEL_PLANNING_V1_DATABASE_URL is required outside local/test environments")
    else:
        store = InMemoryPlatformStore()
    execution_mode = (
        os.getenv(
            "TRAVEL_PLANNING_V1_EXECUTION_MODE",
    ...
    if execution_mode not in {"inline", "external"}:
        raise RuntimeError("TRAVEL_PLANNING_V1_EXECUTION_MODE must be inline or external")
    ...
    rag_database_url = os.getenv("TRAVEL_PLANNING_RAG_DATABASE_URL", "").strip() or database_url
    if rag_database_url:
        from travel_planning_agent_runtime.rag import (
            KnowledgeService,
    ...
    from travel_planning_agent_runtime.providers import build_default_provider_gateway
    from travel_planning_agent_runtime.runtime_v2 import (
        LocalA2AAgentMesh,
    ...
    execution_profile = (
        os.getenv(
            "TRAVEL_PLANNING_V1_EXECUTION_PROFILE",
    ...
    if execution_profile != "v2_research_planning":
        raise RuntimeError("TRAVEL_PLANNING_V1_EXECUTION_PROFILE must be v2_research_planning")
