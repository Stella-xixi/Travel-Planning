...
class OIDCSettings:
    ...
    def from_environment(cls) -> "OIDCSettings | None":
        values = {
            "issuer": os.getenv("TRAVEL_PLANNING_OIDC_ISSUER", "").strip(),
            "audience": os.getenv("TRAVEL_PLANNING_OIDC_AUDIENCE", "").strip(),
            "jwks_url": os.getenv("TRAVEL_PLANNING_OIDC_JWKS_URL", "").strip(),
        }
        if not any(values.values()):
            return None
        if not all(values.values()):
            raise RuntimeError("TRAVEL_PLANNING OIDC issuer, audience, and JWKS URL must be set together")
        algorithms = tuple(
            item.strip()
            for item in os.getenv("TRAVEL_PLANNING_OIDC_ALGORITHMS", "RS256,ES256").split(",")
            if item.strip()
        )
        return cls(
            ...
            tenant_claim=os.getenv("TRAVEL_PLANNING_OIDC_TENANT_CLAIM", "tenant_id").strip(),
            roles_claim=os.getenv("TRAVEL_PLANNING_OIDC_ROLES_CLAIM", "roles").strip(),
            authorization_epoch_claim=os.getenv(
                "TRAVEL_PLANNING_OIDC_AUTHORIZATION_EPOCH_CLAIM",
                "authorization_epoch",
            ).strip(),
        )
...
def install_oidc_authenticator(app: Any) -> None:
    ...
    if settings is not None:
        app.state.travel_planning_v1_authenticator = OIDCAuthenticator(settings)
...
def _is_development_environment() -> bool:
    ...
    environments = {
        ...
        os.getenv("TRAVEL_PLANNING_DEPLOYMENT_ENV", "").strip().lower(),
    }
...
def _trusted_development_bff(request: Request) -> bool:
    ...
    expected = os.getenv("TRAVEL_PLANNING_V1_DEV_BFF_SECRET", "").strip()
    provided = request.headers.get("X-Travel-Planning-Dev-BFF-Secret", "").strip()
    return (
        len(expected) >= 32
...
async def require_principal(request: Request) -> Principal:
    ...
    Production deployments must install `app.state.travel_planning_v1_authenticator`
    ...
    authenticator = getattr(request.app.state, "travel_planning_v1_authenticator", None)
    if authenticator is not None:
        result = authenticator(request)
        principal = await result if inspect.isawaitable(result) else result
        if not isinstance(principal, Principal):
            raise RuntimeError("travel_planning_v1_authenticator must return Principal")
        return principal

    dev_enabled = os.getenv("TRAVEL_PLANNING_V1_DEV_AUTH", "").strip().lower() in {"1", "true", "yes"}
    ...
    if dev_enabled and _is_development_environment() and trusted_peer:
        tenant_id = request.headers.get("X-Travel-Planning-Dev-Tenant", "").strip()
        user_id = request.headers.get("X-Travel-Planning-Dev-User", "").strip()
        if tenant_id and user_id:
            roles = frozenset(
                item.strip()
                for item in request.headers.get("X-Travel-Planning-Dev-Roles", "owner").split(",")
                if item.strip()
            )
