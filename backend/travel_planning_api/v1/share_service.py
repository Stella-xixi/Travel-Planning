"""Capability-secured immutable Trip sharing for Travel-Planning V1."""
...
from travel_planning_contracts.artifacts import (
    PublicGeoPoint,
    PublicPlace,
    ShareDay,
    ShareSnapshot,
...
from travel_planning_contracts.common import ActorRef, ArtifactRef, ArtifactType
...
class ShareService:
    ...
    def __init__(
        ...
        *,
        ...
        configured = pepper if pepper is not None else os.getenv("TRAVEL_PLANNING_SHARE_PEPPER", "")
        if not configured and environment in {"staging", "preprod", "production", "prod"}:
            raise RuntimeError("TRAVEL_PLANNING_SHARE_PEPPER is required in secure deployments")
        self._pepper = (configured or _base64url(secrets.token_bytes(32))).encode("utf-8")
        if len(self._pepper) < 32:
            raise RuntimeError("TRAVEL_PLANNING_SHARE_PEPPER must contain at least 32 bytes")
    ...
    async def _scope(connection: AsyncConnection, tenant_id: str) -> None:
        await connection.execute(
            select(func.set_config("travel-planning.tenant_id", tenant_id, True))
        )
    ...
    async def _resolve_public_tenant(self, public_id: str) -> str | None:
        ...
        async with self.engine.connect() as connection:
            return (
                await connection.execute(
                    select(func.travel_planning_resolve_share_tenant(public_id))
                )
            ).scalar_one_or_none()
