...
def normalize_async_database_url(database_url: str) -> str:
    ...
    if not normalized.startswith("postgresql+psycopg://"):
        raise ValueError("Travel-Planning V1 durable storage requires a PostgreSQL psycopg DSN")
    return normalized
...
class PostgresPlatformStore:
    ...
    async def _scope_tenant(connection: AsyncConnection, principal: Principal) -> None:
        ...
        await connection.execute(
            select(func.set_config("travel-planning.tenant_id", principal.tenant_id, True))
        )
