      ...
      <section className="mx-auto max-w-6xl space-y-6">
        <div className="rounded-2xl border bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            管理文化路线、餐饮混排、预算优先、效率优先和动态重规划能力。所有方案基于本地北京 POI、UGC 评论特征和静态营业时间生成，并明确标注估算边界。
          </p>
          <div className="mt-5 flex gap-3">
            <Button asChild><Link href="/">创建路线任务</Link></Button>
            <Button asChild variant="outline"><Link href="/data-platform">查看 POI 数据</Link></Button>
          </div>
        </div>
          ...
          {routeCapabilities.map((capability) => (
            <article key={capability.id} className="rounded-xl border bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {capability.tags.map((tag) => <Badge key={tag} variant="outline">{tag}</Badge>)}
              </div>
