...
export default async function EvalPlatformPage() {
  ...
  const checks = [
    ['路线 POI 串联', '每条方案至少 3 个 POI，输出顺序、停留时长、转移时间和风险说明。'],
    ['混合类别覆盖', '混合路线至少 1 个餐饮 + 2 个文化/娱乐 POI；不吃饭路线不得强塞餐饮。'],
    ['预算与时长', '输出 within_budget / within_duration，并在超限时显式标记风险。'],
    ['UGC 证据', '展示排队风险、性价比、亲子友好和环境质量等本地 UGC 信号。'],
    ['动态重规划', '支持添加、删除、替换、保留地点和预算/时长调整，不要求用户重说完整需求。'],
    ['响应时间', '本地生成目标小于 10 秒，热缓存接口目标小于 500ms。'],
  ];
      ...
      <section className="mx-auto max-w-5xl space-y-6">
        <div className="rounded-2xl border bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-600">
            当前本地数据：{health.counts.planner_entities} 个规划 POI，
            {health.counts.review_aggregates} 条 UGC 聚合特征。
          </p>
        </div>
