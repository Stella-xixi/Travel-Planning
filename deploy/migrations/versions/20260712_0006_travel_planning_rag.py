"""add Travel-Planning V1 provenance-aware hybrid knowledge retrieval
...
def upgrade() -> None:
    ...
    op.execute(
        """
        DO $travel_planning_vector_extension$
        BEGIN
          BEGIN
          ...
          EXCEPTION
            WHEN insufficient_privilege OR undefined_file OR feature_not_supported THEN
              RAISE WARNING
                'pgvector unavailable; Travel-Planning RAG will run lexical-only until installed';
          END;
        END
        $travel_planning_vector_extension$;
        """
    )
    ...
    op.execute(
        """
        DO $travel_planning_vector_schema$
        BEGIN
          IF EXISTS (
            SELECT 1
              FROM pg_extension
          ...
          ELSIF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') THEN
            RAISE WARNING
              'pgvector version is older than 0.8; Travel-Planning RAG will run lexical-only';
          END IF;
        END
        $travel_planning_vector_schema$;
        """
    )
    ...
    for table_name in visibility_tables:
        ...
        op.execute(
            sa.text(
                ...
                "tenant_id = current_setting('travel-planning.tenant_id', true)) "
    ...
    for table_name in ("v1_knowledge_ingestion_keys", "v1_knowledge_publications"):
        ...
        op.execute(
            sa.text(
                ...
                "current_setting('travel-planning.tenant_id', true))) "
    ...
    op.execute(
        ...
        "USING (tenant_id = current_setting('travel-planning.tenant_id', true)) "
        "WITH CHECK (tenant_id = current_setting('travel-planning.tenant_id', true))"
    )
    op.execute(
        """
        DO $travel_planning_vector_rls$
        BEGIN
          IF EXISTS (
            SELECT 1
              FROM pg_class c
