...
def upgrade() -> None:
    ...
    op.create_table(
        "travel_planning_share_public_lookup",
    ...
    op.execute(sa.text("REVOKE ALL ON TABLE travel_planning_share_public_lookup FROM PUBLIC"))
    op.execute(
        sa.text(
            """
            CREATE FUNCTION travel_planning_resolve_share_tenant(p_public_id text)
            RETURNS text
            LANGUAGE sql
            STABLE
            SECURITY DEFINER
            ...
            AS $$
              SELECT tenant_id
                FROM public.travel_planning_share_public_lookup
    ...
    op.execute(
        sa.text("REVOKE ALL ON FUNCTION travel_planning_resolve_share_tenant(text) FROM PUBLIC")
    )
...
def downgrade() -> None:
    ...
    op.execute(sa.text("DROP FUNCTION IF EXISTS travel_planning_resolve_share_tenant(text)"))
    op.drop_index(
        "ix_v1_share_idempotency_keys_expires",
    ...
    op.drop_table("travel_planning_share_public_lookup")
