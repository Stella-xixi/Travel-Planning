"""Dependency-aware health probe for Travel-Planning V1 Python containers."""
...
def check_redis() -> None:
    ...
    redis_url = os.environ.get("TRAVEL_PLANNING_REDIS_URL", "").strip()
    if not redis_url:
        raise RuntimeError("TRAVEL_PLANNING_REDIS_URL is required")
...
def main() -> int:
    ...
    if args.mode == "api":
        ...
        check_database("TRAVEL_PLANNING_V1_DATABASE_URL")
        check_redis()
    else:
        check_database("TRAVEL_PLANNING_OUTBOX_DATABASE_URL")
        check_redis()
    return 0
