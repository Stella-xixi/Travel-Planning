...
const CLIENT_ID = "travel-planning-web";
const CLIENT_SECRET = "oidc-client-secret";
const KID = "travel-planning-test-key";
let privateKey: KeyObject;
let publicJwk: Record<string, unknown>;

function configureOidc(): void {
  process.env.TRAVEL_PLANNING_API_ORIGIN = "http://api.internal:38083";
  process.env.TRAVEL_PLANNING_PUBLIC_ORIGIN = PUBLIC_ORIGIN;
  process.env.TRAVEL_PLANNING_DEPLOYMENT_ENV = "staging";
  process.env.TRAVEL_PLANNING_BFF_DEV_AUTH = "0";
  process.env.TRAVEL_PLANNING_ACCESS_TOKEN_COOKIE_NAME = "__Host-travel_planning_access_token";
  process.env.TRAVEL_PLANNING_OIDC_ISSUER = ISSUER;
  process.env.TRAVEL_PLANNING_OIDC_AUTHORIZATION_URL = `${ISSUER}authorize`;
  process.env.TRAVEL_PLANNING_OIDC_TOKEN_URL = `${ISSUER}token`;
  process.env.TRAVEL_PLANNING_OIDC_END_SESSION_URL = `${ISSUER}logout`;
  process.env.TRAVEL_PLANNING_OIDC_JWKS_URL = `${ISSUER}jwks`;
  process.env.TRAVEL_PLANNING_OIDC_CLIENT_ID = CLIENT_ID;
  process.env.TRAVEL_PLANNING_OIDC_CLIENT_SECRET = CLIENT_SECRET;
  process.env.TRAVEL_PLANNING_OIDC_COOKIE_KEY = Buffer.alloc(32, 19).toString("base64url");
  process.env.TRAVEL_PLANNING_OIDC_ALGORITHMS = "RS256";
}
...
function accessJwt(subject = "user-1"): string {
  ...
  return signedJwt({ sub: subject, exp: now + 600, iat: now, aud: "travel-planning-api", iss: ISSUER });
}
...
async function beginLogin(): Promise<{
  ...
  const flowCookie = response.cookies.get("__Host-travel_planning_oidc_flow")?.value ?? "";
  return {
...
describe("built-in OIDC Authorization Code + PKCE session", () => {
  it("creates a fixed PKCE authorization request and hardened transient cookie", async () => {
    ...
    expect(cookie).toContain("__Host-travel_planning_oidc_flow=");
  ...
  it("fails closed when secure OIDC configuration is missing or uses HTTP", async () => {
    delete process.env.TRAVEL_PLANNING_OIDC_TOKEN_URL;
    const { GET } = await import("@/app/api/auth/login/route");
    expect(() => GET(new NextRequest(`${PUBLIC_ORIGIN}/api/auth/login`))).toThrow(
      /TRAVEL_PLANNING_OIDC_TOKEN_URL/,
    );

    vi.resetModules();
    configureOidc();
    process.env.TRAVEL_PLANNING_OIDC_TOKEN_URL = "http://identity.example.com/token";
  ...
  it("rejects state tampering before token exchange", async () => {
    ...
    const request = new NextRequest(
      ...
      { headers: { Cookie: `__Host-travel_planning_oidc_flow=${login.flowCookie}` } },
    );
  ...
  it("exchanges once, validates nonce/signature, and rejects replay", async () => {
    ...
    expect(success.cookies.get("__Host-travel_planning_access_token")?.value).toBe(accessToken);
    const encryptedRefresh = success.cookies.get("__Host-travel_planning_refresh")?.value ?? "";
  ...
  it("rotates refresh tokens server-side, retries once, and never exposes them", async () => {
    ...
    const refreshCookie = callback.cookies.get("__Host-travel_planning_refresh")?.value ?? "";
    ...
    const request = new NextRequest(`${PUBLIC_ORIGIN}/api/v1/trips`, {
      headers: {
        Cookie: `__Host-travel_planning_access_token=${firstAccess}; __Host-travel_planning_refresh=${refreshCookie}`,
      },
    });
    ...
    expect(response.headers.get("set-cookie")).toContain("__Host-travel_planning_refresh=v1.");
  });
    ...
    const response = POST(new NextRequest(
      `${PUBLIC_ORIGIN}/api/auth/logout?returnTo=${encodeURIComponent("https://evil.example/")}`,
      {
        ...
        headers: {
          ...
          Cookie: "__Host-travel_planning_csrf=csrf-token-with-enough-entropy",
        },
      },
    ));
  ...
  it("returns token-free session status with private no-store headers", async () => {
    ...
    const response = await GET(new NextRequest(`${PUBLIC_ORIGIN}/api/auth/session`, {
      headers: { Cookie: `__Host-travel_planning_access_token=${token}` },
    }));
