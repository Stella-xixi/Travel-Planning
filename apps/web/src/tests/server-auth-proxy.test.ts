...
function configureBase(): void {
  ...
  process.env.TRAVEL_PLANNING_PUBLIC_ORIGIN = "https://travel.example.com";
  process.env.TRAVEL_PLANNING_ACCESS_TOKEN_COOKIE_NAME = "__Host-travel_planning_access_token";
  process.env.TRAVEL_PLANNING_OIDC_ISSUER = "https://identity.example.com/";
  process.env.TRAVEL_PLANNING_OIDC_AUTHORIZATION_URL = "https://identity.example.com/authorize";
  process.env.TRAVEL_PLANNING_OIDC_TOKEN_URL = "https://identity.example.com/token";
  process.env.TRAVEL_PLANNING_OIDC_END_SESSION_URL = "https://identity.example.com/logout";
  process.env.TRAVEL_PLANNING_OIDC_JWKS_URL = "https://identity.example.com/jwks";
  process.env.TRAVEL_PLANNING_OIDC_CLIENT_ID = "travel-planning-web";
  process.env.TRAVEL_PLANNING_OIDC_CLIENT_SECRET = "test-client-secret";
  process.env.TRAVEL_PLANNING_OIDC_COOKIE_KEY = Buffer.alloc(32, 7).toString("base64url");
}
...
describe("server-only BFF authentication", () => {
  ...
  it("converts only the trusted HttpOnly access-token cookie to upstream Bearer", async () => {
    ...
    const request = new NextRequest("https://travel.example.com/api/v1/trips", {
      headers: {
        ...
        Cookie: `__Host-travel_planning_access_token=${trustedJwt}`,
        "X-Travel-Planning-Dev-User": "attacker",
      },
    });
    ...
    expect(headers.has("X-Travel-Planning-Dev-User")).toBe(false);
  });
  ...
  it("does not forward malformed cookie or client Authorization", async () => {
    ...
    const request = new NextRequest("https://travel.example.com/api/v1/trips", {
      headers: {
        Authorization: "Bearer browser-controlled-token",
        Cookie: "__Host-travel_planning_access_token=not-a-jwt",
      },
    });
    const { proxyV1Request } = await import("@/shared/server/proxy");
  ...
  it("sends the shared dev proxy secret only in an explicit local deployment", async () => {
    process.env.TRAVEL_PLANNING_DEPLOYMENT_ENV = "local";
    process.env.TRAVEL_PLANNING_BFF_DEV_AUTH = "1";
    process.env.TRAVEL_PLANNING_BFF_DEV_TENANT = "local-tenant";
    process.env.TRAVEL_PLANNING_BFF_DEV_USER = "local-user";
    process.env.TRAVEL_PLANNING_BFF_DEV_ROLES = "owner";
    process.env.TRAVEL_PLANNING_V1_DEV_BFF_SECRET = "a".repeat(32);
    const upstreamFetch = vi.fn<typeof fetch>().mockResolvedValue(
      Response.json({ items: [] }),
    );
    vi.stubGlobal("fetch", upstreamFetch);
    ...
    const [, init] = upstreamFetch.mock.calls[0] as [URL, RequestInit];
    const headers = new Headers(init.headers);
    expect(headers.get("X-Travel-Planning-Dev-User")).toBe("local-user");
    expect(headers.get("X-Travel-Planning-Dev-BFF-Secret")).toBe("a".repeat(32));
  ...
  it("allowlists read-only Provider metadata without forwarding a Provider key", async () => {
    ...
    expect(headers.get("X-Travel-Planning-Dev-BFF-Secret")).toBe("a".repeat(32));
    expect(headers.has("TRAVEL_PLANNING_AMAP_WEB_KEY")).toBe(false);
  });
  ...
  it("allowlists versioned Artifact reads, edits, and idempotent commands", async () => {
    ...
    const mutationHeaders = {
      ...
      Cookie: "__Host-travel_planning_csrf=csrf-test-token",
    };
