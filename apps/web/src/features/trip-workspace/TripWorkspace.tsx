...
export function TripWorkspace({ tripId }: { tripId: string }) {
  ...
  useEffect(() => {
    try {
      setRailCollapsed(window.localStorage.getItem("travel-planning.workspace.rail-collapsed.v1") === "1");
      setInsightCollapsed(window.localStorage.getItem("travel-planning.workspace.insight-collapsed.v1") === "1");
    } catch {
  ...
  function updateRailCollapsed(collapsed: boolean): void {
    ...
    try {
      window.localStorage.setItem(
        "travel-planning.workspace.rail-collapsed.v1",
        collapsed ? "1" : "0",
      );
    } catch {
  ...
  function updateInsightCollapsed(collapsed: boolean): void {
    ...
    try {
      window.localStorage.setItem(
        "travel-planning.workspace.insight-collapsed.v1",
        collapsed ? "1" : "0",
      );
    } catch {
      // Keep the in-memory interaction functional in privacy mode.
  ...
  if (pageState !== "ready" || !trip) {
    return (
      <main className="workspace-gate">
        <Link href="/" className="brand"><span className="brand-mark"><Icons.Route /></span><span>Travel-Planning</span></Link>
        {pageState === "loading" ? (
