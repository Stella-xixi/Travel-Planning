...
export function PublicShare({ publicId }: { publicId: string }) {
  ...
  return (
    <main className="public-share">
      <header><span className="run-kicker">TRAVEL_PLANNING · READ ONLY</span><h1>{snapshot.title}</h1><p>{snapshot.destination.display_name} · {snapshot.date_window.start_date} → {snapshot.date_window.end_date}</p></header>
