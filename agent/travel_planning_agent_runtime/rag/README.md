...
- 数据库：`TRAVEL_PLANNING_RAG_DATABASE_URL`，未设置时回退读取
  `TRAVEL_PLANNING_V1_DATABASE_URL`；运行时不会 `create_all`。
- 生产 embedding provider 由 composition root 从
  `TRAVEL_PLANNING_EMBEDDING_ENDPOINT`、`TRAVEL_PLANNING_EMBEDDING_API_KEY`、
  `TRAVEL_PLANNING_EMBEDDING_MODEL` 注入；接口采用 OpenAI-compatible
