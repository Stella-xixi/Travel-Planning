...
def build_embedding_provider_from_env() -> EmbeddingProvider | None:
    ...
    endpoint = os.getenv("TRAVEL_PLANNING_EMBEDDING_ENDPOINT", "").strip()
    model_id = os.getenv("TRAVEL_PLANNING_EMBEDDING_MODEL", "").strip()
    api_key = os.getenv("TRAVEL_PLANNING_EMBEDDING_API_KEY", "").strip()
    configured = [bool(endpoint), bool(model_id)]
    if not any(configured):
        return None
    if not all(configured):
        raise RuntimeError(
            "TRAVEL_PLANNING_EMBEDDING_ENDPOINT and TRAVEL_PLANNING_EMBEDDING_MODEL must be configured together"
        )
    environment = os.getenv("ENVIRONMENT", "dev").strip().lower()
    dimension_raw = os.getenv("TRAVEL_PLANNING_EMBEDDING_DIMENSION", "384").strip()
    timeout_raw = os.getenv("TRAVEL_PLANNING_EMBEDDING_TIMEOUT_SECONDS", "10").strip()
    try:
    ...
    return OpenAICompatibleEmbeddingProvider(
        ...
        model_id=model_id,
        model_version=os.getenv("TRAVEL_PLANNING_EMBEDDING_MODEL_VERSION", model_id).strip(),
        dimension=dimension,
...
class DeterministicHashEmbeddingProvider:
    ...
    model_id = "travel-planning-test-hash"
