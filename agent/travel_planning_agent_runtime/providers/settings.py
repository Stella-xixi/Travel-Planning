...
class ProviderSettings(BaseModel):
    ...
    def from_environment(cls) -> "ProviderSettings":
        """Read only canonical Travel-Planning variables; deprecated key aliases are ignored."""

        raw_key = os.getenv("TRAVEL_PLANNING_AMAP_WEB_KEY", "").strip()
        raw_allowlist = os.getenv(
            "TRAVEL_PLANNING_PROVIDER_ALLOWLIST",
            "amap",
        )
        raw_timeout = os.getenv("TRAVEL_PLANNING_AMAP_HTTP_TIMEOUT_SECONDS", "3.0")
        try:
            timeout = float(raw_timeout)
        except ValueError:
            raise ValueError(
                "TRAVEL_PLANNING_AMAP_HTTP_TIMEOUT_SECONDS must be numeric"
            ) from None
        return cls(
