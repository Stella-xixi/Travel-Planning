    ...
    def __init__(
        ...
        *,
        ...
        if model != "deepseek-v4-flash":
            raise ValueError("Travel-Planning V1 only enables the reviewed deepseek-v4-flash profile")
...
def build_research_directive_generator_from_env() -> ResearchDirectiveGenerator | None:
    ...
    raw_key = os.getenv("TRAVEL_PLANNING_LLM_API_KEY", "").strip()
    if not raw_key:
        return None
    raw_tokens = os.getenv("TRAVEL_PLANNING_LLM_MAX_OUTPUT_TOKENS", "96")
    try:
        max_tokens = int(raw_tokens)
    except ValueError:
        raise RuntimeError("TRAVEL_PLANNING_LLM_MAX_OUTPUT_TOKENS must be an integer") from None
    return DeepSeekResearchDirectiveGenerator(
        api_key=SecretStr(raw_key),
        endpoint=os.getenv(
            "TRAVEL_PLANNING_LLM_ENDPOINT",
            "https://api.deepseek.com/chat/completions",
        ).strip(),
        model=os.getenv("TRAVEL_PLANNING_LLM_MODEL", "deepseek-v4-flash").strip(),
...
def build_grounded_answer_generator_from_env() -> GroundedAnswerGenerator | None:
    ...
    if not raw_key:
        return None
    raw_tokens = os.getenv("TRAVEL_PLANNING_ANSWER_MAX_OUTPUT_TOKENS", "320")
    try:
        max_tokens = int(raw_tokens)
    except ValueError:
        raise RuntimeError("TRAVEL_PLANNING_ANSWER_MAX_OUTPUT_TOKENS must be an integer") from None
