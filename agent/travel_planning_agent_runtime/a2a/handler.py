"""Official A2A SDK RequestHandler adapter for Travel-Planning TaskService."""
...
def _actor(context: ServerCallContext) -> A2AActor:
    actor = context.state.get("travel_planning_actor")
    if not isinstance(actor, A2AActor):
        raise UnsupportedOperationError(message="Authenticated Travel-Planning context is required")
    return actor
...
class TravelPlanningA2ARequestHandler(RequestHandler):
...
__all__ = ["TravelPlanningA2ARequestHandler"]
