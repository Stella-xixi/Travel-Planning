"""Travel-Planning's strict profile constants for the A2A 1.0 boundary."""

from __future__ import annotations

A2A_PROTOCOL_VERSION = "1.0"
TRAVEL_ARTIFACT_EXTENSION_URI = "urn:travel-planning:a2a:extensions:travel-artifact:v1"
...
INPUT_REQUEST_SCHEMA_URI = "urn:travel-planning:a2a:input-request:v1"
INPUT_RESPONSE_SCHEMA_URI = "urn:travel-planning:a2a:input-response:v1"
PUBLIC_ERROR_SCHEMA_URI = "urn:travel-planning:a2a:public-error:v1"


def invocation_schema_uri(agent_interface_id: str) -> str:
    """Return the stable structured-input schema URI for one interface."""

    return f"urn:travel-planning:a2a:input:{agent_interface_id}:v1"


def artifact_schema_uri(contract: str) -> str:
    """Return the stable schema URI for a versioned Travel-Planning contract."""

    name, _, version = contract.partition("@")
    return f"urn:travel-planning:schema:artifact:{name}:v{version}"
