"""PostgreSQL Task/inbox/event adapter for the Travel-Planning A2A persistence port."""
...
class PostgresAgentTaskStore(AgentTaskPersistence):
    ...
    async def _scope_tenant(connection: AsyncConnection, tenant_id: str) -> None:
        ...
        await connection.execute(select(func.set_config("travel-planning.tenant_id", tenant_id, True)))
    ...
    async def replace(
        ...
        *,
        ...
        async with self.engine.begin() as connection:
            ...
            await connection.execute(
                text("SELECT pg_notify('travel_planning_a2a_task_changed', :task_scope)"),
                {
