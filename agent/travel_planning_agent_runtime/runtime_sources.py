...
def build_travel_planning_agent_runtime(*args, **kwargs):
    """Lazily import the graph builder to avoid module-import cycles."""

    from .graph.builder import build_travel_planning_agent_runtime as _build_travel_planning_agent_runtime

    return _build_travel_planning_agent_runtime(*args, **kwargs)
...
def _checkpoint_pool_sizes() -> tuple[int, int]:
    ...
    pool_min = _parse_positive_int(os.getenv("TRAVEL_PLANNING_DB_POOL_MIN", server_config.db_pool_min), 1)
    pool_max = _parse_positive_int(os.getenv("TRAVEL_PLANNING_DB_POOL_MAX", server_config.db_pool_max), 5)
    ...
    *,
    ...
    if backend == "postgres":
        database_url = str(
            ...
            or os.getenv("TRAVEL_PLANNING_POSTGRES_DSN")
...
def build_memory_graph_source(
    *,
    ...
    agent = build_travel_planning_agent_runtime(
        llm,
        tools,
